...
arr = np.loadtxt('data/OMGW19.dat')
krangedirac=[]
OMEGAdirac=[]
for row in arr:
	krangedirac.append(np.float(row[0]))
	OMEGAdirac.append(np.float(row[1])+2*2*1/2)

arr = np.loadtxt('data/OMGW19der.dat')
krangediracder=[]
OMEGAdiracder=[]
for row in arr:
...
data = np.load('data/OmegaGW_of_k_LN_001_w_19_precise.npz')
lst=data.files
...
data = np.load('data/OmegaGW_of_k_LN_005_w_19_precise.npz')
lst=data.files
...
data = np.load('data/OmegaGW_of_k_LN_01_w_19_precise.npz')
lst=data.files
...
plt.ylabel('$\\log_{10}\\Omega_{\\rm GW}(w=1/9)$',fontsize=18)
...
plt.savefig("plots/GW_spectrum_w_19.pdf", format='pdf',transparent=True)
...
first_legend = plt.legend(loc=1,frameon=True,fontsize=16,ncol=2,handlelength=1)

plt.tight_layout()

plt.savefig("plots/GW_spectrum_der_w_19.pdf", format='pdf',transparent=True)
